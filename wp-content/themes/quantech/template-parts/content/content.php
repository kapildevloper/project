...
<div <?php post_class( 'single-blog-post' ); ?> id="post-<?php the_ID(); ?>">
    <?php    
    if ( is_sticky() ) {
        echo '<p class="sticky-label">'.esc_html__( 'Featured', 'quantech' ).'</p>';
    }
    if ( has_post_thumbnail() ) :
        $featured_img_url = get_the_post_thumbnail_url(get_the_ID(),'full');
    ?>           
        <div class="post-thumb">
            <?php the_post_thumbnail('quantech_820x460'); ?>
        </div>
    <?php endif;
    ?>    
    <div class="post-content">
        ...
        <?php endif; ?>
        <p><?php echo strip_shortcodes( quantech_excerpt('quantech_opt', 'blog_excerpt', false) ); ?> </p>
